...
- **CI pipeline**. Пайплайн проверяет тесты, стиль кода, возможно добавляет проверку от sonarqube на качество кода. Ни один МР не должен быть залит, если пайлайн для него не прошел успешно.
...
## Использование прекоммит-хуков (Если ресурсы CI критичны)

Как можно больше работы стоит поручать CI агентам. Задача разработчиков - писать как можно больше кода, и прекоммит хуки будут тормозить вашу работу. В идеале, разработчик должен суметь запушить код любого качества, даже недописанный, а проверки кода - осуществляться только для открытого мердж-реквеста.
